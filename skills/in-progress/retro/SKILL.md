...
- **Coding standards**: should the **reviewer agent** be given a new rule to enforce? Should an existing rule be removed or clarified? The reviewer's rulebook is the `clean-code` skill, so a new or changed rule is a proposed edit to its references, presented to the user; a rule that holds for this repo only goes to the repo's own standards file. _Use when_ the reviewer agent failed to catch a mistake.
...
- The `clean-code` skill: the cross-repository standard, read at review through the `code-review` skill and at implementation through `implement`. It is the single home for rules that hold everywhere.
- `CODING_STANDARDS.md`: this file holds only what is specific to this repo and is read during review, not implementation. Add **navigation pointers** to docs folders if the standards file gets more than 1,000 lines long.
